...
def upgrade() -> None:
    ...
    op.execute(
        "DROP TRIGGER IF EXISTS trg_pii_patterns_notify ON pii.pii_patterns;"
    )
    op.execute(
        "DROP FUNCTION IF EXISTS pii.notify_pii_pattern_changed() CASCADE;"
    )
...
def downgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            'score >= 0 AND score <= 1', name='ck_pii_patterns_score'
        ),
        sa.CheckConstraint(
        ...
        sa.UniqueConstraint(
            'entity_type', 'pattern_name', name='uq_pii_patterns_entity_name'
        ),
        schema='pii',
    )
    op.create_index(
