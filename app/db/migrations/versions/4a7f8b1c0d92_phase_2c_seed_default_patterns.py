...
SEED_PATTERNS: list[dict[str, object]] = [
    ...
    {
        "entity_type": "KR_PHONE_LAND",
        "pattern_name": "land_seoul_hyphen",
        "regex": r"(?<!\d)02-\d{3,4}-\d{4}(?!\d)",
        "score": 0.85,
        "context_words": ["전화", "tel", "유선"],
    },
    {
        "entity_type": "KR_PHONE_LAND",
        "pattern_name": "land_regional_hyphen",
        "regex": r"(?<!\d)0(3[1-3]|4[1-4]|5[1-5]|6[1-4])-\d{3,4}-\d{4}(?!\d)",
        "score": 0.85,
        "context_words": ["전화", "tel"],
    },
    {
        "entity_type": "KR_PHONE_LAND",
        "pattern_name": "land_intl",
        "regex": r"\+82[-\s]?(2|3[1-3]|4[1-4]|5[1-5]|6[1-4])[-\s]?\d{3,4}[-\s]?\d{4}",
        "score": 0.85,
        "context_words": ["전화", "tel"],
    },
    {
        "entity_type": "KR_PHONE_INET",
        "pattern_name": "phone_070",
        "regex": r"(?<!\d)070-\d{3,4}-\d{4}(?!\d)",
        "score": 0.8,
        "context_words": ["인터넷", "전화"],
    },
    {
        "entity_type": "KR_PHONE_INET",
        "pattern_name": "phone_050",
        "regex": r"(?<!\d)050\d-\d{3,4}-\d{4}(?!\d)",
        "score": 0.75,
        "context_words": ["안심번호", "050"],
    },
    {
        "entity_type": "KR_FAX",
        "pattern_name": "fax_hyphen",
        "regex": r"(?<!\d)0\d{1,2}-\d{3,4}-\d{4}(?!\d)",
        "score": 0.4,
    ...
    # ── KR_BANK_ACCOUNT extra layouts ──────────────────────────────────────
    {
        ...
        "pattern_name": "krbank_3_6_3",
        "regex": r"(?<!\d)\d{3}-\d{6}-\d{3}(?!\d)",
        "score": 0.85,
        "context_words": ["계좌", "입금", "송금"],
    },
    {
        "entity_type": "KR_BANK_ACCOUNT",
        "pattern_name": "krbank_4_2_7",
        "regex": r"(?<!\d)\d{4}-\d{2}-\d{7}(?!\d)",
        "score": 0.85,
        "context_words": ["계좌", "우체국", "post"],
    },
    {
        "entity_type": "KR_BANK_ACCOUNT",
        "pattern_name": "krbank_6_2_2_3",
        "regex": r"(?<!\d)\d{6}-\d{2}-\d{2}-\d{3}(?!\d)",
        "score": 0.8,
        "context_words": ["계좌", "농협"],
    },
    {
        "entity_type": "KR_VIRTUAL_ACCOUNT",
        "pattern_name": "krvacc_16",
        "regex": r"(?<!\d)\d{16}(?!\d)",
        "score": 0.5,
    ...
    {
        "entity_type": "KR_FOREIGN_REG",
        "pattern_name": "foreign_rrn",
        "regex": r"(?<!\d)\d{6}-[5-8]\d{6}(?!\d)",
        "score": 0.8,
        "context_words": ["외국인", "alien", "registration"],
    },
    {
        "entity_type": "KR_HEALTH_INS",
        "pattern_name": "health_ins_11",
        "regex": r"(?<!\d)\d-\d{10}(?!\d)",
        "score": 0.7,
        "context_words": ["건강보험", "보험증", "건강"],
    },
    {
        "entity_type": "KR_CORP_REG",
        "pattern_name": "corp_reg_13",
        "regex": r"(?<!\d)\d{6}-\d{7}(?!\d)",
        "score": 0.6,
    ...
    # ── Vehicle plates ─────────────────────────────────────────────────────
    {
        "entity_type": "KR_VEHICLE_PLATE",
        "pattern_name": "plate_old",
        "regex": r"(?<![가-힣\d])\d{2}[가-힣]\d{4}(?![가-힣\d])",
        "score": 0.8,
        "context_words": ["차량", "번호판", "자동차"],
    },
    {
        "entity_type": "KR_VEHICLE_PLATE",
        "pattern_name": "plate_new",
        "regex": r"(?<![가-힣\d])\d{3}[가-힣]\d{4}(?![가-힣\d])",
        "score": 0.8,
        "context_words": ["차량", "번호판", "자동차"],
    },
    # ── Postal / address ───────────────────────────────────────────────────
    {
        "entity_type": "KR_ZIPCODE",
        "pattern_name": "zip_5",
        "regex": r"(?<!\d)\d{5}(?!\d)",
        "score": 0.3,
    ...
    {
        "entity_type": "KR_CARD_VALIDITY",
        "pattern_name": "card_validity",
        "regex": r"(?<!\d)(0[1-9]|1[0-2])/(\d{2})(?!\d)",
        "score": 0.4,
        "context_words": ["유효기간", "valid", "expiry"],
    },
    {
        "entity_type": "KR_CARD_CVC",
        "pattern_name": "card_cvc",
        "regex": r"(?<!\d)\d{3,4}(?!\d)",
        "score": 0.2,
    ...
    # ── Date of birth ──────────────────────────────────────────────────────
    {
        ...
        "pattern_name": "dob_dash",
        "regex": r"(?<!\d)(19|20)\d{2}-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01])(?!\d)",
        "score": 0.5,
        "context_words": ["생년월일", "생일", "DOB", "출생"],
    },
    {
        "entity_type": "DATE_OF_BIRTH",
        "pattern_name": "dob_dot",
        "regex": r"(?<!\d)(19|20)\d{2}\.(0[1-9]|1[0-2])\.(0[1-9]|[12]\d|3[01])(?!\d)",
        "score": 0.5,
        "context_words": ["생년월일", "생일", "DOB", "출생"],
    },
    {
        "entity_type": "DATE_OF_BIRTH",
        "pattern_name": "dob_compact",
        "regex": r"(?<!\d)(19|20)\d{2}(0[1-9]|1[0-2])(0[1-9]|[12]\d|3[01])(?!\d)",
        "score": 0.4,
        "context_words": ["생년월일", "생일"],
    },
    # ── Network identifiers ────────────────────────────────────────────────
    {
    ...
    },
    {
        ...
        "pattern_name": "email_kr_domain",
        "regex": r"[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.kr\b",
        "score": 0.9,
    ...
    # ── Older / legacy formats ────────────────────────────────────────────
    {
        ...
        "pattern_name": "krpass_legacy_3letter",
        "regex": r"(?<![A-Za-z0-9])[A-Z]{3}\d{6}(?![A-Za-z0-9])",
        "score": 0.5,
        "context_words": ["여권", "passport"],
    },
    {
        "entity_type": "KR_DRIVERLICENSE",
        "pattern_name": "krdl_plain",
        "regex": r"(?<!\d)\d{12}(?!\d)",
        "score": 0.4,
    ...
    {
        "entity_type": "KR_NAME_HINT",
        "pattern_name": "korean_name_3char",
    ...
    },
    {
        ...
        "pattern_name": "emp_id_alpha_digits",
        "regex": r"(?<![A-Z0-9])[A-Z]{1,3}\d{4,6}(?![A-Z0-9])",
        "score": 0.4,
        "context_words": ["사번", "employee", "직원번호"],
    },
    {
        "entity_type": "KR_ACCOUNT_TOKEN",
        "pattern_name": "account_uuidish",
        "regex": r"(?<![A-Fa-f0-9-])[A-Fa-f0-9]{8}-[A-Fa-f0-9]{4}-"
        r"[A-Fa-f0-9]{4}-[A-Fa-f0-9]{4}-[A-Fa-f0-9]{12}(?![A-Fa-f0-9-])",
        "score": 0.5,
        "context_words": ["토큰", "token", "uuid"],
    },
]
...
def downgrade() -> None:
    ...
    conn = op.get_bind()
    conn.execute(sa.text("DELETE FROM pii.pii_pattern_history WHERE changed_by = 'system:seed'"))
    conn.execute(sa.text("DELETE FROM pii.pii_patterns WHERE created_by = 'system:seed'"))
